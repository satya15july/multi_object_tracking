...
class Tracker:
    ...
    def __init__(self, metric=None, matching_threshold=None, max_disappeared=10):
        ...
        if self.matching_threshold is None:
    ...
    def set_width_height(self, width, height):
        self.metric.set_width_height(width, height)
    ...
    def register(self, state):
        ...
        if self.debug_flag:
            print("Tracker::register, state: {}".format(state))
            print("Tracker::register, Before Register.. self.tracked: {}".format(self.tracked))
        self.tracked[self.nextTrackID] = KalmanTrack(state)
        self.disappeared[self.nextTrackID] = 0
        self.nextTrackID += 1
        if self.debug_flag:
            print("Tracker::register, After Register.. self.tracked: {}".format(self.tracked))
    ...
    def deregister(self, object_id):
        ...
        if self.debug_flag:
            print("Tracker::register, Before deregister.. object_id: {}".format(object_id))
        del self.tracked[object_id]
        del self.disappeared[object_id]
        if self.debug_flag:
            print("Tracker::deregister, After deregister.. self.tracked: {}".format(self.tracked))
            print("Tracker::register, After deregister.. self.disappeared: {}".format(self.disappeared.keys()))
    ...
    def handle_no_detections(self):
        ...
        and deletes objects if their counter is expired
         ...
         """
        if self.debug_flag:
            print("Tracker::handle_no_detections")
    ...
    def project(self):
        ...
        if self.debug_flag:
            print("Tracker::project, tracks: {}".format(tracks))
        ...
        if self.debug_flag:
            print("Tracker::project, return tracks: {}".format(tracks))
        return tracks

    def linear_assignment(self, D, track_ids, detections):
        if self.debug_flag:
            print("Tracker::linear_assignment")
            print("D: {}".format(D))
            print("track_ids: {}".format(track_ids))
            print("detections: {}".format(detections))
            print("matching_threshold: {}".format(self.matching_threshold))
        print("matching_threshold: {}".format(self.matching_threshold))
    ...
    def crop_bbox_from_frame(frame, bboxes):
        ...
        if config.DEBUG_FLAG:
            print("Tracker::crop_bbox_from_frame")
...
class KalmanTracker(Tracker):
    ...
    def update(self, detections):
        ...
        """
        if self.debug_flag:
            print("KalmanTracker::update, detections: {}".format(detections))
        ...
        if len(self.tracked) == 0:
            if self.debug_flag:
                print("KalmanTracker::update, New tracks,Register")
        ...
        # otherwise, check to see how the new detections relate to current tracks
            ...
            if self.debug_flag:
                print("KalmanTracker::update, associate detection: {}".format(detections))
            self.associate(detections)

        if self.debug_flag:
            print("KalmanTracker::update, return project: {}".format(self.project()))
